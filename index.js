...
async function testNetworkManagerContract(peerNodesfileName) {
    ...
    var networkManagerAddress = "0x2e75bEcbc66Ec5219bb6E8df340059dB23af0202";
    ...
    for(let nodeIndex = 0; nodeIndex < noOfNodes; nodeIndex++) {
        //encodedABI = await nmContract.methods.getNodeDetails(0).encodeABI();
        //var result = await utils.getData(ethAccountToUse,networkManagerAddress,encodedABI,web3);
        let result = await nmContract.methods.getNodeDetails(nodeIndex).call();
        console.log("HostName ", result.nodeName,"\Role ", result.role, "\IP Address ", result.ipAddress, "\nPort ", result.port, "\nPublic Key ", result.publicKey, "\nEnode ", result.enode);
    }
